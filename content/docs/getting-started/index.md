...
Although these are the default schemes, you can also create your own. Refer to the  section for details.
...
The `pre` parameter allows you to place an icon from  on the menu entry. This parameter can be used with `name` parameter or by itself. If you want to use multiple menu entries with just icons please set the `identifier`parameter otherwise Hugo will default to the naming tag as the id and probably not display all the menu entries.
